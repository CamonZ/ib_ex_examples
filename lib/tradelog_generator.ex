defmodule IbExExamples.TradelogGenerator do
  use GenServer
  ...
  alias IbExExamples.TradelogGenerator.Transaction
  ...
  def handle_continue(:start_client, state) do
    case Client.start_link(trace_messages: true) do
      {:ok, pid} ->
  ...
  def handle_continue(:build_transactions, state) do
    ...
    contract_names =
      Enum.reduce(contracts_to_fetch, %{}, fn symbol, acc -> Map.put(acc, symbol, "") end)
  ...
  def handle_continue(:write_file, state) do
    ...
    require Logger
    Logger.info("Finished generating tradelog file")
  ...
  def handle_cast({:message_received, %CommissionReport{} = msg}, state) do
    {:noreply,
     %{
       state
       | commission_report_messages:
           Map.put(state.commission_report_messages, msg.execution_id, msg)
     }}
  end
  ...
  @impl true
  def handle_cast(
        {:message_received, %MatchingSymbols.SymbolSamples{contracts: contracts}},
        state
      ) do
    contract =
      Enum.find(contracts, fn contract_description ->
        contract_description.contract.security_type == "STK" and
  ...
  def handle_cast(:write_anyway, state) do
    {:noreply, state, {:continue, :write_file}}
  end
